...
function statusBadge(status: Doc<"interviews">["status"]): ReactElement {
  const variant = status === "published" ? "default" : "secondary";
  return <Badge variant={variant}>{status}</Badge>;
}

export function InterviewsAdminTable(): ReactElement {
  const staffReady = useConvexStaffSessionReady();
  const rows =
    useQuery(api.interviews.getAllInterviews, staffReady ? {} : "skip") ?? [];
  const verifyHuman = useRecaptchaGate();
  const remove = useMutation(api.interviews.deleteInterview);
        ...
        {rows.map((row: Doc<"interviews">) => (
          <TableRow key={row._id}>
            ...
            <TableCell className="flex flex-wrap items-center justify-end gap-2 max-sm:justify-start">
              <Link
                href={`/admin/interviews/${row._id}/edit`}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  void (async () => {
                    try {
                      await verifyHuman("admin_interview_delete");
                    } catch {
