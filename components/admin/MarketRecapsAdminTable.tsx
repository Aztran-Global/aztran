...
function statusBadge(status: Doc<"marketRecaps">["status"]): ReactElement {
  const variant = status === "published" ? "default" : "secondary";
  return <Badge variant={variant}>{status}</Badge>;
}

export function MarketRecapsAdminTable(): ReactElement {
  const staffReady = useConvexStaffSessionReady();
  const rows =
    useQuery(api.marketRecaps.getAllMarketRecaps, staffReady ? {} : "skip") ??
    [];
  const verifyHuman = useRecaptchaGate();
  const remove = useMutation(api.marketRecaps.deleteMarketRecap);
        ...
        {rows.map((row: Doc<"marketRecaps">) => (
          <TableRow key={row._id}>
            ...
            <TableCell className="flex flex-wrap items-center justify-end gap-2 max-sm:justify-start">
              <Link
                href={`/admin/market-recaps/${row._id}/edit`}
              ...
              <Button
                ...
                onClick={() => {
                  if (!confirm("Delete this market recap?")) return;
                  void (async () => {
                    try {
                      await verifyHuman("admin_market_recap_delete");
                    } catch {
