...
export function MarketRecapCard({
  ...
  coverUrl,
  className,
}: MarketRecapCardProps): ReactElement {
  const thumb =
    coverUrl ?? youtubePosterUrl(marketRecap.youtubeVideoId, "hq");

  return (
    <Link
      href={`/insights/market-recaps/${marketRecap.slug}`}
      className={cn(
