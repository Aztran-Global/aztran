...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  try {
    const slugs = await serverFetchQuery(api.marketRecaps.getAllMarketRecapSlugs);
...
export async function generateMetadata({
  ...
  const doc = await serverFetchQuery(api.marketRecaps.getMarketRecapBySlug, {
    slug,
  });
  if (!doc) {
    return { title: "Market Recap" };
  }
  return createPageMetadata({
    title: doc.seoTitle ?? doc.title,
    description: doc.seoDescription ?? doc.summary,
    path: `/insights/market-recaps/${slug}`,
    type: "article",
  });
}

export default async function MarketRecapDetailPage({
  params,
  ...
  let related: Doc<"marketRecaps">[] = [];
  try {
    related = await serverFetchQuery(api.marketRecaps.getRelatedMarketRecaps, {
      slug,
      limit: 3,
    });
  } catch {
      ...
      {related.length > 0 ? (
        <div className="mx-auto mt-20 max-w-container border-t border-[color-mix(in_srgb,var(--color-silver)_45%,transparent)] px-4 pt-16 dark:border-[color-mix(in_srgb,var(--color-silver)_22%,transparent)] md:px-8">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {related.map((r: Doc<"marketRecaps">) => (
              <MarketRecapCard key={r._id} marketRecap={r} />
            ))}
          </div>
